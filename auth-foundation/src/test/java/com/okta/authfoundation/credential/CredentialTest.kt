...
class CredentialTest {
    ...
    @Test
    fun `getDefaultAsync returns null if no default credential is set`() =
        runTest {
            assertThat(Credential.getDefaultAsync()).isNull()
        }

    @Test
    fun `getDefaultAsync returns the default credential`() =
        runTest {
            val credential = oktaRule.createCredential(createToken())
            val credentialDataSource =
                mockk<CredentialDataSource> {
    ...
    @Test
    fun `allIds returns ids of all available credentials`() =
        runTest {
    ...
    @Test
    fun `get metadata of credential`() =
        runTest {
            val testId = "id1"
    ...
    @Test
    fun `set metadata sets new metadata for credential`() =
        runTest {
            val testId = "id1"
    ...
    @Test
    fun `with returns token with given id`() =
        runTest {
            val testId = "testId"
    ...
    @Test
    fun `find returns tokens matching the expression`() =
        runTest {
            val credential = mockk<Credential>()
    ...
    @Test
    fun `store creates a new credential`() =
        runTest {
            val credential = mockk<Credential>()
            val credentialDataSource =
                mockk<CredentialDataSource> {
    ...
    @Test
    fun `setTags changes credential metadata`() =
        runTest {
    ...
    @Test fun testGetTokenFlow(): Unit =
        runTest {
    ...
    @Test
    fun `delete clears default credential if deleted credential is default`() =
        runTest {
            val token = createToken()
            val credential = oktaRule.createCredential(token = token, credentialDataSource = credentialDataSource)
            Credential.setDefaultAsync(credential)
    ...
    @Test fun testRemoveIsNoOpAfterRemove(): Unit =
        runTest {
            val token = createToken()
            val credential = oktaRule.createCredential(token = token, credentialDataSource = credentialDataSource)
            credential.delete()
    ...
    @Test fun testReplaceTokenIsNoOpAfterRemove(): Unit =
        runTest {
            val token = createToken()
            val credential = oktaRule.createCredential(token = token, credentialDataSource = credentialDataSource)
            credential.delete()
    ...
    @Test fun testReplaceTokenEmitsEventAfterRemove(): Unit =
        runTest {
            val token = createToken()
            val credential = oktaRule.createCredential(token = token, credentialDataSource = credentialDataSource)
            credential.delete()
    ...
    @Test fun testRevokeAccessToken(): Unit =
        runTest {
            val token = createToken()
    ...
    @Test fun testRevokeRefreshToken(): Unit =
        runTest {
    ...
    @Test fun testRevokeDeviceSecret(): Unit =
        runTest {
    ...
    @Test fun testRevokeTokenWithNullRefreshTokenReturnsError(): Unit =
        runTest {
    ...
    @Test fun testRevokeTokenWithNullDeviceSecretReturnsError(): Unit =
        runTest {
            val credential = oktaRule.createCredential(token = createToken())
    ...
    @Test fun testRevokeAllTokensWithOnlyAccessToken(): Unit =
        runTest {
            val token = createToken()
            val credential = oktaRule.createCredential(token = token, oauthClient = client)
    ...
    @Test fun testRevokeAllTokensWithRefreshToken(): Unit =
        runTest {
            val token = createToken(refreshToken = "exampleRefreshToken")
            val credential = oktaRule.createCredential(token = token, oauthClient = client)
            credential.revokeAllTokens()
    ...
    @Test fun testRevokeAllTokensWithDeviceSecret(): Unit =
        runTest {
            val token = createToken(deviceSecret = "exampleDeviceSecret")
            val credential = oktaRule.createCredential(token = token, oauthClient = client)
            credential.revokeAllTokens()
    ...
    @Test fun testRevokeAllTokensWithRefreshTokenAndDeviceSecret(): Unit =
        runTest {
    ...
    @Test fun testRevokeAllTokensWithFailedRefreshTokenReturnsError(): Unit =
        runTest {
    ...
    @Test fun testRefreshWithNoRefreshToken(): Unit =
        runTest {
            val credentialDataSource = mockk<CredentialDataSource>(relaxed = true)
    ...
    @Test fun testRefreshTokenForwardsError(): Unit =
        runTest {
    ...
    @Test fun testRefreshToken(): Unit =
        runTest {
    ...
    @Test fun testRefreshTokenWithRealOAuth2Client(): Unit =
        runTest {
            val credential =
                oktaRule.createCredential(
    ...
    @Test fun testRefreshTokenWithRealOAuth2ClientPreservesRefreshToken(): Unit =
        runTest {
            val credential =
                oktaRule.createCredential(
    ...
    @Test fun testRefreshTokenWithRealOAuth2ClientEmitsEvent(): Unit =
        runTest {
            val credential =
                oktaRule.createCredential(
                    token = createToken(refreshToken = "exampleRefreshToken")
    ...
    @Test fun testRefreshTokenPreservesRefreshToken(): Unit =
        runTest {
    ...
    @Test fun testRefreshTokenPreservesDeviceSecret(): Unit =
        runTest {
            coEvery { client.refreshToken(any()) } returns OAuth2ClientResult.Success(createToken(refreshToken = "newRefreshToken"))
    ...
    @Test fun testParallelRefreshToken(): Unit =
        runTest {
    ...
    @Test fun testSerialRefreshToken(): Unit =
        runTest {
    ...
    @Test fun testReplaceToken(): Unit =
        runTest {
            val credential = oktaRule.createCredential(token = token, credentialDataSource = credentialDataSource)
    ...
    @Test fun testReplaceTokenKeepsPreviousDeviceSecret(): Unit =
        runTest {
            val credentialDataSource = mockk<CredentialDataSource>(relaxed = true)
            val credential =
                oktaRule.createCredential(
    ...
    @Test fun testReplaceTokenKeepsStoresDeviceSecret(): Unit =
        runTest {
            val credentialDataSource = mockk<CredentialDataSource>(relaxed = true)
            val credential =
                oktaRule.createCredential(
    ...
    @Test fun testIdTokenWithNullIdTokenReturnsNullJwt(): Unit =
        runTest {
    ...
    @Test fun testIdToken(): Unit =
        runTest {
            val credential =
                oktaRule.createCredential(
                    token =
                        createToken(
                            idToken = "eyJraWQiOiJGSkEwSEdOdHN1dWRhX1BsNDVKNDJrdlFxY3N1XzBDNEZnN3BiSkxYVEhZIiwiYWxnIjoiUlMyNTYifQ..tT8aKK4r8yFcW9KgVtZxvjXRJVzz-_rve14CVtpUlyvCTE1yj20wmPS0z3-JirI9xXgt5KeNPYqo3Wbv8c9XY_HY3hsPQdILYpPsUkf-sctmzSoKC_dTbs5xe8uKSgmpMrggfUAWrNPiJt9Ek2p7GgP64Wx79Pq5vSHk0yWlonFfXut5ahpSfqWilmYlvLr8gFbqoLnAJfl4ZbTY8pPw_aQgCdcQ-ImHRu-8bCSCtbFRzZB-SMJFLfRF2kmx0H-QF855wUODTuUSydkff-BKb-8wnbqWg0R9NvRdoXhEybv8TXXZY3cQqgolWLAyiPMrz07n0q_UEjAilUiCjn1f4Q"
                        )
                )
    ...
    @Test fun testMalformedIdToken(): Unit =
        runTest {
    ...
    @Test fun accessTokenIfValidReturnsNullWithExpiredToken(): Unit =
        runTest {
    ...
    @Test fun accessTokenIfValidReturnsAccessToken(): Unit =
        runTest {
    ...
    @Test
    fun `accessTokenIfValid returns null if idToken is null and expiry time is passed`() =
        runTest {
            val accessToken = "exampleAccessToken"
    ...
    @Test
    fun `accessTokenIfValid returns access token even if idToken is null`() =
        runTest {
            val accessToken = "exampleAccessToken"
            val credential = oktaRule.createCredential(token = createToken(accessToken = accessToken))
            assertThat(credential.getAccessTokenIfValid()).isEqualTo(accessToken)
    ...
    @Test
    fun `accessTokenIfValid returns null if idToken does not contain required payload and issuedAt is past expiry time`(): Unit =
        runTest {
            @Serializable
            class EmptyClaims
    ...
    @Test
    fun `accessTokenIfValid returns accessToken if idToken does not contain required payload but issuedAt is not expired`(): Unit =
        runTest {
            @Serializable
            class EmptyClaims
    ...
    @Test fun getValidAccessTokenReturnsAccessToken(): Unit =
        runTest {
            val accessToken = "exampleAccessToken"
            val idTokenClaims = IdTokenClaims(issuedAt = oktaRule.clock.currentTime)
            val idToken =
    ...
    @Test fun getValidAccessTokenReturnsNullWhenAccessTokenExpiredAndNoRefreshToken(): Unit =
        runTest {
            val idTokenClaims = IdTokenClaims(issuedAt = oktaRule.clock.currentTime - MOCK_TOKEN_DURATION)
            val idToken =
                oktaRule
    ...
    @Test fun getValidAccessTokenRefreshesWhenAccessTokenExpired(): Unit =
        runTest {
            ...
            val expiredIdToken =
                oktaRule
                    .createOAuth2Client()
                    .createJwtBuilder()
                    .createJwt(claims = expiredIdTokenClaims)
                    .rawValue
            ...
            val validIdToken =
                oktaRule
                    .createOAuth2Client()
                    .createJwtBuilder()
                    .createJwt(claims = validIdTokenClaims)
                    .rawValue
            oktaRule.enqueue(path("/oauth2/default/v1/token")) { response ->
    ...
    @Test fun getValidAccessTokenReturnsNullWhenRefreshedAccessTokenExpired(): Unit =
        runTest {
            val expiredIdTokenClaims = IdTokenClaims(issuedAt = oktaRule.clock.currentTime - MOCK_TOKEN_DURATION)
            val expiredIdToken =
                oktaRule
    ...
    @Test fun testGetUserInfoNeedsRefresh(): Unit =
        runTest {
            val expiredAccessToken = "expiredAccessToken"
            val validAccessToken = "exampleAccessToken"
            val expiredIdTokenClaims = IdTokenClaims(issuedAt = oktaRule.clock.currentTime - MOCK_TOKEN_DURATION)
    ...
    @Test fun testGetUserInfoNoToken(): Unit =
        runTest {
            val credential = oktaRule.createCredential(token)
            expireToken()
    ...
    @Test fun testIntrospectNullRefreshToken(): Unit =
        runTest {
    ...
    @Test fun testIntrospectNullIdToken(): Unit =
        runTest {
            val credential = oktaRule.createCredential(token = createToken(idToken = null))
    ...
    @Test fun testIntrospectNullDeviceSecret(): Unit =
        runTest {
    ...
    @Test fun testIntrospectRefreshToken(): Unit =
        runTest {
    ...
    @Test fun testIntrospectAccessToken(): Unit =
        runTest {
            coEvery { client.introspectToken(any(), any()) } returns OAuth2ClientResult.Success(OidcIntrospectInfo.Inactive)
    ...
    @Test fun testIntrospectIdToken(): Unit =
        runTest {
            coEvery { client.introspectToken(any(), any()) } returns OAuth2ClientResult.Success(OidcIntrospectInfo.Inactive)
    ...
    @Test fun testIntrospectDeviceSecret(): Unit =
        runTest {
            coEvery { client.introspectToken(any(), any()) } returns OAuth2ClientResult.Success(OidcIntrospectInfo.Inactive)
            val token = createToken(deviceSecret = "exampleDeviceSecret")
            val credential = oktaRule.createCredential(token = token, oauthClient = client)
    ...
    @Test fun testIntrospectAccessTokenNetworkError(): Unit =
        runTest {
    ...
    @Test fun testReplaceTokenExceptionCausesErrorResponse(): Unit =
        runTest {
            val credentialDataSource = mockk<CredentialDataSource>()
            coEvery {
