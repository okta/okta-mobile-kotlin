...
class RoomTokenStorageTest {
    ...
    @Test
    fun `empty storage returns empty for all IDs`() =
        runTest {
            assertThat(roomTokenStorage.allIds()).isEmpty()
        }

    @Test
    fun `get metadata for non-existent id returns null`() =
        runTest {
            assertThat(roomTokenStorage.metadata("non-existent-id")).isNull()
        }

    @Test
    fun `setMetadata for a token with non-existent id fails`() =
        runTest {
    ...
    @Test
    fun `add token fails if a token with the same id already exists`() =
        runTest {
            roomTokenStorage.add(token, tokenMetadata)
    ...
    @Test
    fun `add token fails if token id and metadata id don't match`() =
        runTest {
            val exception =
    ...
    @Test
    fun `remove non-existent id does nothing`() =
        runTest {
            roomTokenStorage.add(token, tokenMetadata)
    ...
    @Test
    fun `replace token with non-existent id`() =
        runTest {
            assertFailsWith<NoSuchElementException> {
    ...
    @Test
    fun `get token fails when no token with id exists`() =
        runTest {
            assertFailsWith<NoSuchElementException> {
    ...
    @Test
    fun `add and get biometric token`() =
        runTest {
