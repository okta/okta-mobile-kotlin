...
class NetworkingTest {
    ...
    @Test fun testPerformRequestErrorStatusCodesCallMapper(): Unit =
        runTest {
            oktaRule.enqueue(
                path("/test")
            ) { response ->
            ...
            val result =
                oktaRule.createOAuth2Client().performRequest(JsonObject.serializer(), request, { true }) {
                    it["foo"]!!.jsonPrimitive.content
                }
            assertThat((result as OAuth2ClientResult.Success<String>).result).isEqualTo("bar")
        }

    @Test fun testPerformRequestExceptionInMapper(): Unit =
        runTest {
            oktaRule.enqueue(
                path("/test")
            ) { response ->
            ...
            val result =
                oktaRule.createOAuth2Client().performRequest<JsonObject, String>(JsonObject.serializer(), request, { true }) {
    ...
    @Test fun testPerformRequestNetworkFailure(): Unit =
        runTest {
    ...
    @Test fun testPerformRequestErrorStatusReturnsError(): Unit =
        runTest {
            oktaRule.enqueue(
                path("/test")
            ) { response ->
    ...
    @Test fun testPerformRequestErrorStatusReturnsErrorDeserialized(): Unit =
        runTest {
            oktaRule.enqueue(
                path("/test")
            ) { response ->
    ...
    @Test fun testPerformRequestNonJson(): Unit =
        runTest {
            oktaRule.enqueue(
                path("/test")
            ) { response ->
    ...
    @Test fun testPerformRequestFailsWhileReadingResponse(): Unit =
        runTest {
            oktaRule.enqueue(
                path("/test")
            ) { response ->
    ...
    @Test fun testPerformRequestFailsWhileReadingErrorResponse(): Unit =
        runTest {
            oktaRule.enqueue(
                path("/test")
            ) { response ->
    ...
    @Test fun testPerformRequestHasNoTagWithNoCredential(): Unit =
        runTest {
            ...
            val result =
                oAuth2Client.performRequest(JsonObject.serializer(), request) {
                    it["foo"]!!.jsonPrimitive.content
                }
            assertThat((result as OAuth2ClientResult.Success<String>).result).isEqualTo("bar")
    ...
    @Test fun testPerformRequestEnsuresTheCoroutineIsActiveBeforeMakingNetworkRequest(): Unit =
        runTest {
            val request =
                Request
                    .Builder()
    ...
    @Test fun testPerformRequestRawResponseEnsuresTheCoroutineIsActiveBeforeMakingNetworkRequest(): Unit =
        runTest {
            val request =
                Request
                    .Builder()
    ...
    @Test fun testPerformRequestRawResponse(): Unit =
        runTest {
            oktaRule.enqueue(
                path("/test")
            ) { response ->
    ...
    @Test fun testPerformRequestWithNon429ErrorHasNoRetry(): Unit =
        runTest {
            oktaRule.enqueue(path("/test")) { response ->
                response.setResponseCode(401)
                response.setBody("""{"foo":"bar"}""")
    ...
    @Test fun `do not retry request when verbatim response is expected`() =
        runTest {
            oktaRule.enqueue(path("/test")) { response ->
            ...
            val result =
                oktaRule.createOAuth2Client().performRequest(JsonObject.serializer(), request, shouldAttemptJsonDeserialization = { true }) {
                    it["foo"]!!.jsonPrimitive.content
                }
            assertThat((result as OAuth2ClientResult.Success<String>).result).isEqualTo("bar")
        }

    @Test fun testPerformRequestWithNon429ErrorAndRateLimitHeadersDoesNotRetry() =
        runTest {
            oktaRule.enqueue(path("/test")) { response ->
                response.setResponseCode(401)
    ...
    @Test fun testPerformRequestDoesNotRetryOnNetworkFailure(): Unit =
        runTest {
            oktaRule.enqueue(path("/test")) { response ->
                response.socketPolicy = SocketPolicy.DISCONNECT_AT_START
            }
    ...
    @Test fun testPerformRequestRetryOn429ErrorAndStopOnSuccess(): Unit =
        runTest {
            oktaRule.enqueue(path("/test")) { response ->
                response.setResponseCode(429)
                setRateLimitHeaders(response)
    ...
    @Test fun testPerformRequestRetryOn429FollowedByDifferentError(): Unit =
        runTest {
            oktaRule.enqueue(path("/test")) { response ->
                response.setResponseCode(429)
                setRateLimitHeaders(response)
    ...
    @Test fun testPerformRequestRetryContainsRetryForHeader(): Unit =
        runTest {
    ...
    @Test fun testPerformRequestRetryDoesNotContainRetryForHeaderIfRequestIdMissing(): Unit =
        runTest {
            oktaRule.enqueue(
                path("/test"),
    ...
    @Test fun testPerformRequestRetryContainsRetryCountHeader(): Unit =
        runTest {
            val retries = 3
            oktaRule.enqueue(
                path("/test"),
    ...
    @Test fun testPerformRequestRetryAfterMinimumDelay(): Unit =
        runTest {
    ...
    @Test fun testPerformRequestRetryAtResetTime(): Unit =
        runTest {
    ...
    @Test fun testPerformRequestRetryUpToMaxRetriesOn429ErrorResponse(): Unit =
        runTest {
            val maxRetries = 5
    ...
    @Test fun testPerformRequestRetryCancelsOnCoroutineCancellation(): Unit =
        runTest {
    ...
    @Test fun testPerformRequestRetryWithInvalidDateHeaderShouldReturn429Response(): Unit =
        runTest {
            oktaRule.enqueue(path("/test")) { response ->
                response.setResponseCode(429)
    ...
    @Test fun testPerformRequestRetryWithInvalidRateLimitResetHeaderShouldReturn429Response(): Unit =
        runTest {
            oktaRule.enqueue(path("/test")) { response ->
                response.setResponseCode(429)
                setRateLimitHeaders(response)
    ...
    @Test fun testPerformRequestRetrySendsAnEventForEach429Response(): Unit =
        runTest {
            val maxRetries = 10
            val events = mutableListOf<RateLimitExceededEvent>()
            val rateLimitEventHandler =
                object : EventHandler {
    ...
    @Test fun testPerformRequestRetryChangesDelayBasedOnHandledEvent(): Unit =
        runTest {
    ...
    @Test fun testPerformRequestRetryChangesMaxRetriesBasedOnHandledEvent(): Unit =
        runTest {
            val events = mutableListOf<RateLimitExceededEvent>()
            val rateLimitEventHandler =
                object : EventHandler {
    ...
    @Test fun testPerformRequestRetryClosesUnusedResponsesAfterEmittingEvent(): Unit =
        runTest {
            val events = mutableListOf<RateLimitExceededEvent>()
            val rateLimitEventHandler =
                object : EventHandler {
    ...
    @Test fun `prioritize custom okhttp interceptors when making requests`() =
        runTest {
            ...
            oAuth2Client.performRequest(String.serializer(), request) { }
