...
class TokenData(
     ...
     *
     * Defaults to `clock.now - expiresIn`, which represents the worst-case assumption that
     * the token was issued a full lifetime ago (i.e., it may be near expiry). Callers that
     * know the actual issuance time should pass it explicitly.
     */
    ...
    /**
     * Equality is based on the token payload fields only (tokenType, expiresIn, accessToken,
     * scope, refreshToken, idToken, deviceSecret, issuedTokenType).
     *
     * [id], [configuration], and [issuedAt] are excluded because [id] is a storage key
     * (not semantic identity), and [configuration]/[issuedAt] are context — two tokens
     * with the same payload from different clients represent the same authorization grant.
     */
