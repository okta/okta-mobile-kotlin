...
class RedirectCoordinatorTest {
    ...
    @Test fun testEmitNullIsCancel(): Unit =
        testScope.runTest {
            val startTime = currentTime
            val resultDeferred = listenRedirectAsync()
            subject.emit(null)
    ...
    @Test fun testEmitNullIsReplacedOnSecondEmit(): Unit =
        testScope.runTest {
            val startTime = currentTime
            val resultDeferred = listenRedirectAsync()
            subject.emit(null)
    ...
    @Test fun testEmitNullIsCancelAfterDebounceTime(): Unit =
        testScope.runTest {
            val startTime = currentTime
            val resultDeferred = listenRedirectAsync()
            subject.emit(null)
    ...
    @Test fun testEmitNullCancelsInitialization(): Unit =
        testScope.runTest {
            val countDownLatch = CountDownLatch(1)
    ...
    @Test fun testEmitWithoutListening(): Unit =
        testScope.runTest {
            subject.emit(null)
            advanceUntilIdle()
        }

    @Test fun testEmitErrorWithoutListening(): Unit =
        testScope.runTest {
            subject.emitError(IllegalStateException("From Test!"))
            advanceUntilIdle()
        }
    ...
    @Test fun testRunInitializationFunctionReturnsErrorIfInitializeIsNotCalled(): Unit =
        testScope.runTest {
    ...
    @Test fun testRunInitializationFunctionClearsInitializer(): Unit =
        testScope.runTest {
            val webAuthenticationProvider = mock<WebAuthenticationProvider>()
    ...
    @Test fun testInitializeStartsForegroundActivity(): Unit =
        testScope.runTest {
    ...
    @Test fun testLaunchWebAuthenticationProvider(): Unit =
        testScope.runTest {
            val webAuthenticationProvider =
                mock<WebAuthenticationProvider> {
                    on { launch(any(), any()) } doReturn null
