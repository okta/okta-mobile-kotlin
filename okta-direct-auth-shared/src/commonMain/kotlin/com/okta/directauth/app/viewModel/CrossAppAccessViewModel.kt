...
/**
 * Scopes requested by Cross App Access's own dedicated sign-in — distinct from
 * [OAuth2FlowViewModel]'s own default scope (which also requests `email`). `internal` so
 * `App.kt`'s `buildXaaIdpClient()` can reuse this exact value for the client's own default scope,
 * rather than duplicating the literal and risking the two silently drifting apart.
 */
internal val XAA_IDP_SCOPE = listOf("openid", "profile", "offline_access")

/** Splits [CrossAppAccessViewModel]'s raw scope text on whitespace; compiled once, not per call. */
private val SCOPE_SPLIT_REGEX = Regex("\\s+")
 ...
 * sample's flows: the ID-JAG exchange must be submitted to the *org's own* authorization server,
 * never a custom one, so it cannot safely reuse a session that some other flow may have
 * established against a custom `authorizationServerId`. See [signIn].
 *
...
class CrossAppAccessViewModel(
    ...
    // Resolved once, here, rather than by re-invoking resolveTargetCredential() from every
    // computeResetState()/buildTarget() call below: this is a "testing only" local.properties (or
    // Android build-config-constant) read, identical in spirit to OAuth2FlowViewModel's own client
    // credential setup — see crossAppAccessTargetCredential's KDoc. Reading it exactly once here
    // avoids re-reading a file (and, if a private_key_jwt PEM is configured, re-parsing it) on
    // every reset() and every exchange()/start()/redeem() attempt. Declared before _state so its
    // value — not a not-yet-initialized default — is what computeResetState() below actually reads.
    private val targetCredential: TargetCredential = resolveTargetCredential()

    // Survives reset(): readyState() below seeds every fresh Ready it builds from this, rather
    // than from Ready's own blank default, so "Try Again"/"Start Over" (both reset()) don't throw
    // away scope text the developer already typed. Declared before _state for the same reason as
    // targetCredential above — computeResetState() below reads it during _state's own
    // initializer, so it must already hold its real (non-null) value by then.
    private var lastScopeInput: String = ""
    ...
    // every other flow's own session. Cleared only by reset(); a failed exchange does not require
    // signing in again.
    ...
    fun selectSubjectKind(kind: SubjectKind) {
        _state.update { current -> (current as? CrossAppAccessState.Ready)?.copy(selectedKind = kind) ?: current }
    }

    /** Updates the raw scope text entered by the user, while [state] is [CrossAppAccessState.Ready]. */
    fun updateScopeInput(text: String) {
        lastScopeInput = text
        _state.update { current -> (current as? CrossAppAccessState.Ready)?.copy(scopeInput = text) ?: current }
    }
    ...
    fun signIn(platformContext: Any?) {
        ...
        cancelAndLaunch {
            ...
            val result = performSignIn(platformContext)
            // performSignIn ultimately runs through a runCatching (platformBrowserLogin, or a
            // test's own fake), which also catches CancellationException — rethrow it here so a
            // sign-in cancelled by reset()/a superseding call doesn't go on to report a fabricated
            // failure (or success) against state a newer attempt already owns.
    ...
    fun exchange() {
        ...
        cancelAndLaunch {
            ...
            val result = flow.exchange(subject, parseScope(ready.scopeInput))
            // See the identical check in signIn(): CrossAppAccessFlowImpl's start()/redeem() run
            // through their own runCatching internally, which also catches CancellationException —
            // rethrow it here so a cancelled attempt never reports a stale outcome.
            currentCoroutineContext().ensureActive()
            result.fold(
                onSuccess = { tokenInfo ->
                    redemptionCount = 1
    ...
    fun start() {
        ...
        cancelAndLaunch {
            ...
            val result = flow.start(subject, parseScope(ready.scopeInput))
            // See the identical check in signIn().
            currentCoroutineContext().ensureActive()
            result.fold(
                onSuccess = { idJag ->
        ...
        cancelAndLaunch {
            ...
            result.fold(
                ...
                onFailure = { error ->
                    ...
                    // Retains idJag: a failed redemption (unlike an expired one, checked above)
                    // doesn't invalidate it — the screen offers redeeming it again without forcing
                    // a full reset() and a fresh ID-JAG.
                    _state.value = toFailedState(error, retainedIdJag = idJag)
                }
            )
        }
    }
    ...
    fun introspect() {
        ...
        cancelAndLaunch {
            ...
            val result = flow.targetClient.introspectToken("access_token", current.tokenInfo.accessToken)
            // See the identical check in signIn().
            currentCoroutineContext().ensureActive()
            result.fold(
                onSuccess = { info ->
                ...
                onFailure = { error ->
                    ...
                    // introspectToken() talks to the resource authorization server directly and
                    // never raises a CrossAppAccessException — routing this through
                    // CrossAppAccessErrors.classify() would misreport a genuine transport/server
                    // failure here as FailureAttribution.CONFIGURATION, which that attribution's
                    // own KDoc reserves for failures before any network request.
                    _state.value =
                        CrossAppAccessState.Failed(
                            FailureAttribution.RESOURCE_SERVER,
                            "The resource authorization server rejected the introspection request: " +
    ...
    private fun currentIdJag(): IdJagAssertion? =
        when (val current = _state.value) {
            ...
            // Lets the screen's "Redeem Again" button retry the exact ID-JAG a failed redemption
            // (not an expired one, which never reaches Failed with one retained — see redeem())
            // left behind, without forcing a full reset() and a fresh identity-provider round trip.
            is CrossAppAccessState.Failed -> current.retainedIdJag
    ...
    private fun buildTarget(client: OAuth2Client): CrossAppAccessTarget {
        val credential = targetCredential
    ...
    private fun toFailedState(
        error: Throwable,
        retainedIdJag: IdJagAssertion? = null,
    ): CrossAppAccessState.Failed {
        val classified = CrossAppAccessErrors.classify(error, lastSelectedKind)
        return CrossAppAccessState.Failed(classified.attribution, classified.message, retainedIdJag)
    }

    private fun computeResetState(): CrossAppAccessState {
        val hasTargetCredential = targetCredential != TargetCredential.None
