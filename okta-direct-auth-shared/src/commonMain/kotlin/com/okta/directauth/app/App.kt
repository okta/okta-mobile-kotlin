...
import com.okta.directauth.app.viewModel.XAA_IDP_SCOPE
...
fun App(appStorage: AppStorage) {
    ...
    // Cross App Access owns its own IdP client and session, entirely separate from every other
    // flow's own client — see CrossAppAccessViewModel's KDoc for why it cannot safely reuse the
    // primary client's session.
 ...
 *
 * Called exactly once, from `remember { }` at the top of [App] — [crossAppAccessIdpCredential]'s
 * "testing only" local.properties (or Android build-config-constant) read happens once for this
 * client's whole lifetime, the same tradeoff [OAuth2FlowViewModel.client]'s own construction-time
 * credential read already makes for the rest of this sample.
 */
