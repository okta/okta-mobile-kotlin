 ...
 *   used to retry with a different subject kind (or scope) after a failure, or to start over
 *   from a successfully obtained [CrossAppAccessState.IdJagObtained] before redeeming it.
    ...
    Column(
        ...
        when (state) {
            ...
            is CrossAppAccessState.IdJagObtained -> {
                IdJagSection(idJag = state.idJag, redemptionCount = state.redemptionCount, onRedeem = onRedeem, onReset = onReset)
            }
            ...
            is CrossAppAccessState.Failed -> {
                FailedSection(state = state, onRedeem = onRedeem, onReset = onReset)
            }
        }
 ...
 * is a read-only preview of a token the sample already holds, not a trust decision. Discards the
 * original header entirely and substitutes a fixed `{"alg":"none"}` one before parsing, so
 * [Jwts]'s unsecured parser accepts a signed JWT without needing (or checking) its key — the
 * original header's actual algorithm is irrelevant here since only the payload is ever displayed,
 * and this way every algorithm is handled alike, not just the ones this sample happens to name.
 * Falls back to a raw dump for access/refresh tokens that are opaque (not JWTs) — Cross App
 * Access does not require any of the three token kinds to be JWTs.
 */
private fun buildTokenClaimsText(rawToken: String): String {
    ...
    return runCatching {
        ...
        val unsecuredJwt = urlSafeNoPad.encode("""{"alg":"none"}""".toByteArray()) + "." + parts[1] + "."
        val claims =
            Jwts
                .parser()
                .unsecured()
...
private fun IdJagSection(
    ...
    Column {
        ...
        Spacer(modifier = Modifier.height(Dimens.spaceSmall))
        // Abandons this ID-JAG and returns to Ready — same reset() as a Failed screen's "Try
        // Again", just reachable from a successful step-by-step first step too, e.g. to pick a
        // different subject kind or scope before redeeming.
        OutlinedButton(onClick = onReset, modifier = Modifier.fillMaxWidth()) {
            Text(text = XaaStrings.START_OVER_BUTTON)
        }
    }
}
...
private fun FailedSection(
    ...
    Column {
        ...
        Spacer(modifier = Modifier.height(Dimens.spaceMedium))
        // Redeeming again reuses the exact ID-JAG a failed (not expired) redemption left behind —
        // only offered when the view model actually retained one; see redeem()/currentIdJag().
        if (state.retainedIdJag != null) {
            Button(onClick = onRedeem, modifier = Modifier.fillMaxWidth()) {
                Text(text = XaaStrings.REDEEM_AGAIN_BUTTON)
            }
            Spacer(modifier = Modifier.height(Dimens.spaceSmall))
        ...
        Button(onClick = onReset, modifier = Modifier.fillMaxWidth()) {
            // An exact match against the one message redeem()'s own expiry check ever produces —
            // rather than a fuzzy substring search that could also match an unrelated server error
            // that happens to mention "expired" (e.g. a device code's own expiry).
            Text(text = if (state.serverMessage == XaaStrings.ID_JAG_EXPIRED_NOTE) XaaStrings.RESTART_AFTER_EXPIRY_BUTTON else XaaStrings.TRY_AGAIN_BUTTON)
        }
    }
}
