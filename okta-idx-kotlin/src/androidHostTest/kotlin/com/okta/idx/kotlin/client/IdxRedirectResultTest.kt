...
class IdxRedirectResultTest {
    ...
    @Test fun testRedirectResultInvalidUrl(): Unit =
        runBlocking {
    ...
    @Test fun testRedirectResultMismatchRedirectUri(): Unit =
        runBlocking {
    ...
    @Test fun testRedirectResultInteractionRequired(): Unit =
        runBlocking {
            ...
            val uri =
                Uri.parse(
                    "test.okta.com/login?state=cdef&error=interaction_required&error_description=Your+client+is+configured+to+use+the+interaction+code+flow+and+user+interaction+is+required+to+complete+the+request."
                )
            val client = createClient()
            val redirectResult = client.evaluateRedirectUri(uri) as IdxRedirectResult.InteractionRequired
            assertThat(
                redirectResult.response.remediations
                    .first()
                    .name
            ).isEqualTo("select-authenticator-authenticate")
        }

    @Test fun testRedirectResultInteractionRequiredFailsWithMismatchState(): Unit =
        runBlocking {
            val uri =
                Uri.parse(
                    "test.okta.com/login?state=mismatch&error=interaction_required&error_description=Your+client+is+configured+to+use+the+interaction+code+flow+and+user+interaction+is+required+to+complete+the+request."
                )
            val client = createClient()
    ...
    @Test fun testRedirectResultInteractionRequiredIntrospectError(): Unit =
        runBlocking {
            networkRule.enqueue(path("/idp/idx/introspect")) { response ->
    ...
    @Test fun testRedirectResultInteractionCode(): Unit =
        runBlocking {
    ...
    @Test fun testRedirectResultInteractionCodeWithMaxAge(): Unit =
        runBlocking {
            val body = "grant_type=interaction_code&client_id=test&interaction_code=exampleInteractionCode&code_verifier=abcd"
            networkRule.enqueue(path("/oauth2/default/v1/token"), body(body)) { response ->
                response.testBodyFromFile("client/tokenResponse.json")
    ...
    @Test fun testRedirectResultInteractionCodeErrorParamShowsErrorDescription(): Unit =
        runBlocking {
    ...
    @Test fun testRedirectResultInteractionCodeErrorParamShowsDefaultError(): Unit =
        runBlocking {
    ...
    @Test fun testRedirectResultInteractionCodeStateMismatch(): Unit =
        runBlocking {
    ...
    @Test fun testRedirectResultInteractionCodeExchangeTokensError(): Unit =
        runBlocking {
            val body = "grant_type=interaction_code&client_id=test&interaction_code=exampleInteractionCode&code_verifier=abcd"
            networkRule.enqueue(path("/oauth2/default/v1/token"), body(body)) { response ->
                response.socketPolicy = SocketPolicy.DISCONNECT_AT_START
