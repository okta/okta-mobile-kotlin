...
import com.okta.idx.kotlin.kmp.IdxRemediation.Type
import com.okta.idx.kotlin.kmp.v1.convertBase64UrltoBase64
...
class IdxPollRemediationCapability internal constructor(
     ...
     *
     * @return the [Result] when the state changes.
     */
    suspend fun poll(flow: InteractionCodeFlow): Result<IdxResponse> {
        var result: Result<IdxResponse>
        ...
        do {
            ...
            val remediations = result.getOrNull()?.remediations ?: return result
...
class IdxPollAuthenticatorCapability internal constructor(
    ...
    suspend fun poll(flow: InteractionCodeFlow): Result<IdxResponse> {
        ...
        do {
            ...
            val currentAuthenticator = result.getOrNull()?.authenticators?.current ?: return result
 ...
 *
 * Unlike the legacy `com.okta.idx.kotlin.dto.IdxTotpCapability`, this type does not expose a
 * `Bitmap`-decoding method — decoding [imageData] into a displayable image is a platform-specific
 * consumer concern (e.g. `android.graphics.BitmapFactory` on Android).
 */
...
class IdxWebAuthnRegistrationCapability internal constructor(
    ...
    fun publicKeyCredentialCreationOptions(relyingPartyIdentifier: String? = null): Result<String> =
        runCatching {
            val rootObject = Json.parseToJsonElement(activationData).jsonObject
            val rpObject = rootObject["rp"]?.jsonObject ?: throw IllegalArgumentException("Missing 'rp' object.")
            ...
            val newRpId =
                ...
                } else if (!rpObject.containsKey("id")) {
                    val appId =
                        rpObject["u2fParams"]
                            ?.jsonObject
                            ?.get("appid")
                            ?.jsonPrimitive
                            ?.contentOrNull
                    appId?.let { runCatching { Url(it).host }.getOrNull() }
                } else {
                    null
                }
            ...
            val newRpObject =
                if (newRpId != null) {
                    JsonObject(rpObject.toMutableMap().apply { put("id", JsonPrimitive(newRpId)) })
                } else {
                    rpObject
                }

            JsonObject(rootObject.toMutableMap().apply { put("rp", newRpObject) }).toString()
        }
     ...
     */
    fun withRegistrationResponse(
        remediation: IdxRemediation,
        registrationResponseJson: String,
    ): Result<IdxRemediation> =
        runCatching {
            if (remediation.authenticators.current
                    ?.capabilities
                    ?.get<IdxWebAuthnRegistrationCapability>() == null
            ) {
                throw IllegalArgumentException("This remediation does not have a WebAuthn registration capability.")
            }

            val createCredentialResponseJson = Json.parseToJsonElement(registrationResponseJson).jsonObject
            val response =
                createCredentialResponseJson["response"]?.jsonObject
                    ?: throw IllegalArgumentException("The 'response' field is not present in the create credential response.")
            val attestationObject =
                response["attestationObject"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
                    ?: throw IllegalArgumentException("The 'attestationObject' field is not present in the create credential response.")
            val clientDataJSON =
                response["clientDataJSON"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
...
class IdxWebAuthnAuthenticationCapability internal constructor(
    ...
    fun challengeData(relyingPartyIdentifier: String? = null): Result<String> =
        runCatching {
            val rootObject = Json.parseToJsonElement(_challengeData).jsonObject

            // Determine the final rpId value. If rpId is provided and not blank, use it; otherwise, check for an existing "rpId" in the root.
            // If neither is available, check for "appid" in "extensions" and use its host as the rpId.
            val newRpId =
                if (!relyingPartyIdentifier.isNullOrBlank()) {
                    relyingPartyIdentifier
                } else if (!rootObject.containsKey("rpId")) {
                    val appId =
            ...
            if (newRpId != null) {
                JsonObject(rootObject.toMutableMap().apply { put("rpId", JsonPrimitive(newRpId)) }).toString()
            } else {
                rootObject.toString()
            }
        }
    ...
    fun withAuthenticationResponseJson(
        ...
        runCatching {
            if (remediation.type != Type.CHALLENGE_WEBAUTHN_AUTOFILLUI_AUTHENTICATOR &&
                remediation.authenticators.current
                    ?.capabilities
                    ?.get<IdxWebAuthnAuthenticationCapability>() == null
            ) {
                throw IllegalArgumentException("This remediation does not have a WebAuthn authentication capability.")
            }

            val authenticationResponseObject = Json.parseToJsonElement(authenticationResponseJson).jsonObject
            val response =
                authenticationResponseObject["response"]?.jsonObject
                    ?: throw IllegalArgumentException("The 'response' field is not present in the authentication response.")
            val clientDataJson =
                response["clientDataJson"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
                    ?: response["clientDataJSON"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
                    ?: throw IllegalArgumentException("The 'clientDataJson' field is not present in the authentication response.")
            val authenticatorData =
                response["authenticatorData"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
                    ?: throw IllegalArgumentException("The 'authenticatorData' field is not present in the authentication response.")
            val signature =
                response["signature"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
                    ?: throw IllegalArgumentException("The 'signature' field is not present in the authentication response.")
            val userHandle = response["userHandle"]?.jsonPrimitive?.contentOrNull?.takeIf { it.isNotBlank() }
            ...
            userHandle?.let { remediation.form["credentials.userHandle"]?.apply { value = it } }
