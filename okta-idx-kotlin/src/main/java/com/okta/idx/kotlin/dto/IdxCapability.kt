    ...
    fun publicKeyCredentialCreationOptions(relyingPartyIdentifier: String? = null): Result<String> =
        runCatching {
            ...
            if (!relyingPartyIdentifier.isNullOrBlank()) {
                ...
                val derivedRpId =
                    rpObject.optJSONObject("u2fParams")?.optString("appid")?.let { appId ->
                        try {
                            URI(appId).host
    ...
    fun withRegistrationResponse(
        ...
        runCatching {
            ...
            val attestationObject =
                response.getString("attestationObject").takeIf { it.isNotBlank() }
                    ?: throw IllegalArgumentException("The 'attestationObject' field is not present in the create credential response.")
            val clientDataJSON =
                response.getString("clientDataJSON").takeIf { it.isNotBlank() }
...
class IdxWebAuthnAuthenticationCapability internal constructor(
    ...
    fun challengeData(relyingPartyIdentifier: String? = null): Result<String> =
        runCatching {
            ...
            if (!relyingPartyIdentifier.isNullOrBlank()) {
                ...
                val derivedRpId =
                    rootObject.optJSONObject("extensions")?.optString("appid")?.let { appId ->
                        try {
                            URI(appId).host
                        } catch (_: URISyntaxException) {
                            null
    ...
    fun withAuthenticationResponseJson(
        ...
        runCatching {
            ...
            val clientDataJson =
                response.optString("clientDataJson").takeIf { it.isNotBlank() }
                    ?: response.optString("clientDataJSON").takeIf { it.isNotBlank() } ?: throw IllegalArgumentException("The 'clientDataJson' field is not present in the authentication response.")
            val authenticatorData =
                response.getString("authenticatorData").takeIf { it.isNotBlank() }
                    ?: throw IllegalArgumentException("The 'authenticatorData' field is not present in the authentication response.")
            val signature =
                response.getString("signature").takeIf { it.isNotBlank() }
                    ?: throw IllegalArgumentException("The 'signature' field is not present in the authentication response.")
            val userHandle = response.optString("userHandle").takeIf { it.isNotBlank() }
