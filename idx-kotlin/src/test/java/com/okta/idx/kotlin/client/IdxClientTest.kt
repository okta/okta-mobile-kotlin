...
class IdxClientTest {
    ...
    @Test fun testStart(): Unit = runBlocking {
        ...
        assertThat(clientResult.response.clientContext.interactionHandle).isEqualTo("029ZAB")
    }
    ...
    @Test fun testResume(): Unit = runBlocking {
        ...
        assertThat(resumeResult.response.remediations).hasSize(4)
    }
    ...
    @Test fun testProceed(): Unit = runBlocking {
        ...
        val proceedResult = client.proceed(identifyRemediation) as IdxClientResult.Response<IdxResponse>
        assertThat(proceedResult.response.remediations[1].type).isEqualTo(IdxRemediation.Type.ISSUE)
    }
    ...
    @Test fun testProceedCopiesRemediationValues(): Unit = runBlocking {
        ...
        identifyRemediation["credentials.passcode"]?.value = "example"
        val proceedResult = client.proceed(identifyRemediation) as IdxClientResult.Response<IdxResponse>
        val newIdentifyRemediation = proceedResult.response.remediations[0]
    ...
    @Test fun testExchangeCodes(): Unit = runBlocking {
        ...
        identifyRemediation["credentials.passcode"]?.value = "example"
        val proceedResult = client.proceed(identifyRemediation) as IdxClientResult.Response<IdxResponse>
        val proceedResponse = proceedResult.response
        val issueRemediation = proceedResponse.remediations[1]
        assertThat(issueRemediation.type).isEqualTo(IdxRemediation.Type.ISSUE)

        val tokenResult = client.exchangeCodes(issueRemediation) as IdxClientResult.Response<TokenResponse>
