...
internal class TokenStepHandler(override val request: DirectAuthTokenRequest, override val context: DirectAuthenticationContext) : StepHandler {
    ...
    override suspend fun process(): DirectAuthenticationState = runCatching {
        ...
        return apiResponse.handleErrorResponse(context, httpStatusCode) { apiError ->
            when (val errorCode = DirectAuthenticationErrorCode.fromString(apiError.error)) {
                ...
                AUTHORIZATION_PENDING -> AuthorizationPending(context.clock.currentTimeEpochSecond())
