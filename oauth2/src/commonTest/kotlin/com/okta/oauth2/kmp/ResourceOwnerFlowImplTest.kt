...
class ResourceOwnerFlowImplTest {
    ...
    /**
     * Creates a [ResourceOwnerFlowImpl] backed by a stub [ApiExecutor].
     *
     * The first response is always the discovery response. Subsequent responses are consumed
     * in order for each API call made by the flow.
     */
    private fun createFlowWithScope(
        clientScope: List<String>,
        vararg apiResponses: Pair<Int, String>,
    ): Pair<ResourceOwnerFlowImpl, () -> ApiRequest?> {
        ...
        return ResourceOwnerFlowImpl(client) to { capturedRequest }
    }
    ...
    fun start_SendsCorrectFormParams() =
        runTest {
            ...
            assertEquals("password", params["grant_type"])
            assertEquals("user@example.com", params["username"])
            assertEquals("s3cr3t", params["password"])
            assertEquals("test-client-id", params["client_id"])
        }

    @Test
    fun start_WhenServerReturnsError_ReturnsFailure() =
        runTest {
