...
class DeviceAuthorizationFlowTest {
    ...
    @Test fun testStartWithMinimalJson(): Unit =
        runBlocking {
            val minimalBody =
                """
                {
    ...
    @Test fun testStartWithNoDeviceAuthorizationEndpoints(): Unit =
        runBlocking {
    ...
    @Test fun testStartWithExtraParameters(): Unit =
        runBlocking {
            oktaRule.enqueue(
                method("POST"),
                path("/oauth2/default/v1/device/authorize"),
    ...
    @Test fun testResumeWithNoEndpoints(): Unit =
        runBlocking {
            oktaRule.enqueue(path("/.well-known/openid-configuration")) { response ->
                response.setResponseCode(503)
            }
    ...
    @Test fun testResumeWithNoPolling(): Unit =
        runBlocking {
            oktaRule.enqueue(
                method("POST"),
            ...
            val flow = DeviceAuthorizationFlow()
            val context =
                DeviceAuthorizationFlow.Context(
    ...
    @Test fun testResumeWithPolling(): Unit =
        runBlocking {
            repeat(4) {
                oktaRule.enqueue(
                    method("POST"),
    ...
    @Test fun testResumeWithSlowDown(): Unit =
        runBlocking {
            repeat(4) {
                oktaRule.enqueue(
                    method("POST"),
    ...
    @Test fun testResumeTimeout(): Unit =
        runBlocking {
            repeat(4) {
                oktaRule.enqueue(
                    method("POST"),
    ...
    @Test fun testResumeNetworkError(): Unit =
        runBlocking {
            oktaRule.enqueue(
                method("POST"),
                path("/oauth2/default/v1/token"),
