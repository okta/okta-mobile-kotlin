...
class AuthorizationCodeFlowTest {
    ...
    @Test fun testStartWithNoEndpoints(): Unit =
        runBlocking {
    ...
    @Test fun testStartWithMaxAge(): Unit =
        runBlocking {
    ...
    @Test fun testStartWithMalformedMaxAge(): Unit =
        runBlocking {
            val requestParameters = mutableMapOf<String, String>()
    ...
    @Test fun testStartWithExtraRequestParameters(): Unit =
        runBlocking {
            val authorizationCodeFlow = AuthorizationCodeFlow()
            assertThat(oktaRule.eventHandler).isEmpty()
    ...
    @Test fun testResume(): Unit =
        runBlocking {
            ...
            oktaRule.enqueue(
                ...
                body(
                    "redirect_uri=unitTest%3A%2Flogin&code_verifier=LEadFL0UCCWDlD0cdIiuv7TQfbxOP8OUep0U_xo_3oI&client_id=unit_test_client_id&grant_type=authorization_code&code=D13x1bzHhzG7Q1oxSmCcoQg5wjbJzopF4ua1f8UZiE4"
                )
            ) { response ->
    ...
    @Test fun testResumeWithMaxAge(): Unit =
        runBlocking {
            reset(idTokenValidator)
            oktaRule.enqueue(
                method("POST"),
    ...
    @Test fun testResumeNetworkFailure(): Unit =
        runBlocking {
            oktaRule.enqueue(
                path("/oauth2/default/v1/token")
            ) { response ->
    ...
    @Test fun testResumeRedirectMismatch(): Unit =
        runBlocking {
            val authorizationCodeFlow = AuthorizationCodeFlow()
            val flowContext =
                AuthorizationCodeFlow.Context(
    ...
    @Test fun testResumeStateMismatch(): Unit =
        runBlocking {
            val authorizationCodeFlow = AuthorizationCodeFlow()
            val flowContext =
                AuthorizationCodeFlow.Context(
    ...
    @Test fun testResumeErrorDescription(): Unit =
        runBlocking {
            val authorizationCodeFlow = AuthorizationCodeFlow()
            val flowContext =
                AuthorizationCodeFlow.Context(
