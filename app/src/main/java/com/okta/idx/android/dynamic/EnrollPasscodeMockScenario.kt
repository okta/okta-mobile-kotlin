...
internal object EnrollPasscodeMockScenario {
    fun prepare() {
        ...
        OktaMockWebServer.dispatcher.enqueue(path("/idp/idx/introspect")) { response ->
            response.mockBodyFromFile("enroll_passcode/introspect-response.json")
        }
        OktaMockWebServer.dispatcher.enqueue(path("/idp/idx/challenge/answer")) { response ->
            response.mockBodyFromFile("enroll_passcode/answer-response.json")
        }
        OktaMockWebServer.dispatcher.enqueue(path("/oauth2/v1/token")) { response ->
            response.mockBodyFromFile("enroll_passcode/token-response.json")
        }
        OktaMockWebServer.dispatcher.enqueue(path("/idp/idx/cancel")) { response ->
            response.mockBodyFromFile("enroll_passcode/cancel-response.json")
        }
    }
}
